...
from typing import Callable, Dict, List, Optional, TYPE_CHECKING, Type, TypeVar

from semantic_kernel.ai import (
    AIBackend,
    TextAIBackend,
    ChatAIBackend,
    ImageAIBackend,
...
class KernelConfig:
    def __init__(self) -> None:
        self._text_backends: Dict[str, Callable[["KernelBase"], TextAIBackend]] = {}
        self._chat_backends: Dict[str, Callable[["KernelBase"], ChatAIBackend]] = {}
        self._image_backends: Dict[str, Callable[["KernelBase"], ImageAIBackend]] = {}
        self._embedding_backends: Dict[
            str, Callable[["KernelBase"], EmbeddingAIBackend]
        ] = {}

        self._default_text_backend: Optional[str] = None
        self._default_chat_backend: Optional[str] = None
        self._default_image_backend: Optional[str] = None
    ...
    def get_ai_backend(
        ...
        matching_type = {}
        if type == TextAIBackend:
            service_id = service_id or self._default_text_backend
            matching_type = self._text_backends
        elif type == ChatAIBackend:
            service_id = service_id or self._default_chat_backend
            matching_type = self._chat_backends
        elif type == ImageAIBackend:
            service_id = service_id or self._default_image_backend
            matching_type = self._image_backends
        elif type == EmbeddingAIBackend:
    ...
    def all_image_backends(self) -> List[str]:
        return list(self._image_backends.keys())
    ...
    def add_text_backend(
        self,
        service_id: str,
        backend: Callable[["KernelBase"], TextAIBackend],
        ...
        self._text_backends[service_id] = backend
    ...
    def add_chat_backend(
        self,
        service_id: str,
        backend: Callable[["KernelBase"], ChatAIBackend],
        overwrite: bool = False,
    ) -> "KernelConfig":
        if not service_id:
            raise ValueError("service_id must be a non-empty string")
        ...
        self._chat_backends[service_id] = backend
    ...
    def add_image_backend(
        self,
        service_id: str,
        backend: Callable[["KernelBase"], ImageAIBackend],
        overwrite: bool = False,
    ) -> "KernelConfig":
        if not service_id:
            raise ValueError("service_id must be a non-empty string")
        if not overwrite and service_id in self._image_backends:
            raise ValueError(
                f"Image backend with service_id '{service_id}' already exists"
            )

        self._image_backends[service_id] = backend
        if self._default_image_backend is None:
            self._default_image_backend = service_id
    ...
    def add_embedding_backend(
        self,
        service_id: str,
        backend: Callable[["KernelBase"], EmbeddingAIBackend],
        overwrite: bool = False,
    ) -> "KernelConfig":
        if not service_id:
            raise ValueError("service_id must be a non-empty string")
        ...
        self._embedding_backends[service_id] = backend
    ...
    def set_default_image_backend(self, service_id: str) -> "KernelConfig":
        if service_id not in self._image_backends:
            raise ValueError(
                f"AI backend with service_id '{service_id}' does not exist"
            )
    ...
    def get_image_backend_service_id(self, service_id: Optional[str] = None) -> str:
        if service_id is None or service_id not in self._image_backends:
            if self._default_image_backend is None:
                raise ValueError("No default image backend is set")
            return self._default_image_backend
    ...
    def remove_image_backend(self, service_id: str) -> "KernelConfig":
        if service_id not in self._image_backends:
            raise ValueError(
                f"AI backend with service_id '{service_id}' does not exist"
            )

        del self._image_backends[service_id]
        if self._default_image_backend == service_id:
            self._default_image_backend = next(iter(self._image_backends), None)
        return self
    ...
    def clear_all_image_backends(self) -> "KernelConfig":
        self._image_backends = {}
        self._default_image_backend = None
        return self
